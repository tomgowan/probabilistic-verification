...
freq_gefs = zeros((21,20))
freq_gefs[:,0] = np.arange(0,1.001,.05)
...
for w in range(len(inhouse_data75[:,0])):  # GEFS 75
    for x in range(len(gefs_data75[:,0])):
        if inhouse_data75[w,0] == gefs_data75[x,0]:
            if inhouse_data75[w,0] != 0:
                for i in range(3,186):
                    if 0 < inhouse_data[w,i] < 1000:
                        for t in range(21):
                            if gefs_data75[x,i] == t/20.:
                                freq_gefs[t,1] = freq_gefs[t,1] + inhouse_data75[w,i]
                                freq_gefs[t,2] = freq_gefs[t,2] + 1
                                

for w in range(len(inhouse_data90[:,0])):  # GEFS 90
    for x in range(len(gefs_data90[:,0])):
        if inhouse_data90[w,0] == gefs_data90[x,0]:
            if inhouse_data90[w,0] != 0:
                for i in range(3,186):
                    if 0 < inhouse_data[w,i] < 1000:
                        for t in range(21):
                            if gefs_data90[x,i] == t/20.:
                                freq_gefs[t,4] = freq_gefs[t,4] + inhouse_data90[w,i]
                                freq_gefs[t,5] = freq_gefs[t,5] + 1
...
freq_gefs[:,3] = freq_gefs[:,1]/freq_gefs[:,2]      # GEFS 75     
freq_gefs[:,6] = freq_gefs[:,4]/freq_gefs[:,5]      # GEFS 90
...
####### rows are BSSquart//BSquart//BSSdec//BSdec
bss_gefs= zeros((21,10))


### bssf rows are BS, reliabilty, resolution, uncertatinty, BSS 
bssf_gefs = zeros((10,5))
obquart = (sum(freq_gefs[:,1])/sum(freq_gefs[:,2]))
obdec = (sum(freq_gefs[:,4])/sum(freq_gefs[:,5]))

Bref75 = (obquart)*(1-obquart)

Bref90 = (obdec)*(1-obdec)

for i in range(len(freq_gefs[:,0])):
    ### BS
    bss_gefs[i,0] =((((freq_gefs[i,0]-0)**2)*(freq_gefs[i,2]-freq_gefs[i,1]))+(((freq_gefs[i,0]-1)**2)*(freq_gefs[i,1])))
    bss_gefs[i,1] =((((freq_gefs[i,0]-0)**2)*(freq_gefs[i,5]-freq_gefs[i,4]))+(((freq_gefs[i,0]-1)**2)*(freq_gefs[i,4])))
    
    ### Reliability
    bss_gefs[i,2] = (freq_gefs[i,2]*(freq_gefs[i,0]-freq_gefs[i,3])**2)
    bss_gefs[i,3] = (freq_gefs[i,5]*(freq_gefs[i,0]-freq_gefs[i,6])**2)
    
    
    ### Resolution
    bss_gefs[i,4] = (freq_gefs[i,2]*(freq_gefs[i,3]-obquart)**2)
    bss_gefs[i,5] = (freq_gefs[i,5]*(freq_gefs[i,6]-obdec)**2)
...
##### Upper quartile
bssf_gefs[0,0] = sum(bss_gefs[:,0])/sum(freq_gefs[:,2])
bssf_gefs[1,0] = sum(bss_gefs[:,2])/sum(freq_gefs[:,2])
bssf_gefs[2,0] = sum(bss_gefs[:,4])/sum(freq_gefs[:,2])
bssf_gefs[3,0] = Bref75
bssf_gefs[4,0] = 1-(bssf_gefs[0,0]/Bref75) 

#### Upper decile 
bssf_gefs[0,1] = sum(bss_gefs[:,1])/sum(freq_gefs[:,5])
bssf_gefs[1,1] = sum(bss_gefs[:,3])/sum(freq_gefs[:,5])
bssf_gefs[2,1] = sum(bss_gefs[:,5])/sum(freq_gefs[:,2])
bssf_gefs[3,1] = Bref90
bssf_gefs[4,1] = 1-(bssf_gefs[0,1]/Bref90)
...
ax1.plot(freq_gefs[:,0],freq_gefs[:,3], linewidth = 2, c = 'gold',marker = "o", markeredgecolor = 'none')
...
the_table = plt.table(cellText=[('%.3f' % bssf[0,0],'%.3f' % bssf[1,0],'%.3f' % bssf[2,0],'%.3f' % bssf[3,0],'%.3f' % bssf[4,0]),
                                ('%.3f' % bssf_gefs[0,0],'%.3f' % bssf_gefs[1,0],'%.3f' % bssf_gefs[2,0],'%.3f' % bssf_gefs[3,0],'%.3f' % bssf_gefs[4,0]),
                                ...
                                ('%.3f' % bssf_gefs[0,1],'%.3f' % bssf_gefs[1,1],'%.3f' % bssf_gefs[2,1],'%.3f' % bssf_gefs[3,1],'%.3f' % bssf_gefs[4,1]),
...
gold_line = mlines.Line2D([],[] , color='gold',
                           label='GEFS (20 Mem.)',  linewidth = 2,marker = "o", markeredgecolor = 'none')
...
ax2.plot(freq_gefs[:,0],freq_gefs[:,6], linewidth = 2, c = 'gold',marker = "o", markeredgecolor = 'none')
...
plt.bar(freq_gefs[:,0],freq_gefs[:,2],width = .04, color = 'gold', edgecolor ='none', align='center')
plt.xlim([-0.03,1.03])
plt.ylim([10,100000])
plt.title('GEFS (Upper Quart.)', y = 1.05, fontsize = 10)
plt.ylabel('Num. Samples', fontsize = 10)
...
plt.xlim([-0.01,1.01])
plt.ylim([10,100000])
plt.title('ECMWF (Upper Quart.)', y = 1.05, fontsize = 10)
plt.ylabel('Num. Samples', fontsize = 10)
plt.xlabel('Forecast Prob.',fontsize = 10)
a.set_yscale('log')
plt.xticks(np.arange(0,1.0001,.2))
...
a = plt.axes([.8, .16, .1, .055], axisbg='white')   ## GEFS
plt.bar(freq_gefs[:,0],freq_gefs[:,5],width = .04, color = 'gold', edgecolor ='none',align='center')
plt.xlim([-0.03,1.03])
plt.ylim([10,100000])
plt.title('GEFS (Upper Dec.)', y = 1.05, fontsize = 10)
plt.ylabel('Num. Samples', fontsize = 10)
plt.xlabel('Forecast Prob.',fontsize = 10)
a.set_yscale('log')
plt.xticks()
...
plt.xlim([-0.01,1.01])
plt.ylim([10,100000])
plt.title('ECMWF (Upper Dec.)', y = 1.05, fontsize = 10)
plt.ylabel('Num. Samples', fontsize = 10)
plt.xlabel('Forecast Prob.',fontsize = 10)
a.set_yscale('log')
plt.xticks()
