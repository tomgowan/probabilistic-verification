...
links = ["/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/snotel_precip_2016_2017_qc.csv", 
         "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/ncarens_precip_12Zto12Z_upper85_prob_interp_percentiles_pac_non_bias_corrected.txt",
         "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/ncarens_precip_12Zto12Z_upper95_prob_interp_percentiles_pac_non_bias_corrected.txt",
         "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/sref_precip_12Zto12Z_upper85_prob_interp_percentiles_pac_non_bias_corrected.txt",
         "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/sref_precip_12Zto12Z_upper95_prob_interp_percentiles_pac_non_bias_corrected.txt"]
...
for w in range(len(inhouse_data[:,0])):
    if inhouse_data[w,0] != 0:
        for i in range(3,185):
            if snotel_percent_pac[16] <= inhouse_data[w,i] < 1000 :
                inhouse_data75[w,i] = 1
            elif snotel_percent_pac[16] > inhouse_data[w,i]:
...
for w in range(len(inhouse_data[:,0])):
    if inhouse_data[w,0] != 0:
        for i in range(3,185):
            if snotel_percent_pac[18] <= inhouse_data[w,i] < 1000 :
                inhouse_data90[w,i] = 1
            elif snotel_percent_pac[18] > inhouse_data[w,i]:
...
for w in range(len(inhouse_data75[:,0])):  # NCAR 75
    for x in range(len(ncar_data75[:,0])):
        ...
        ################### PACIFIC ###################            
        if ((regions[0,0] <= ncar_data75[x,1] <= regions[0,1] and regions[0,2] <= ncar_data75[x,2] <= regions[0,3]) or ###Sierra Nevada
            (regions[1,0] <= ncar_data75[x,1] <= regions[1,1] and regions[1,2] <= ncar_data75[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
            (ncar_data75[x,1] >= regions[1,4] or ncar_data75[x,2] <= regions[1,5])):
                        ...
                        if inhouse_data[w,i] < 1000:
                            for t in range(11):
                                if ncar_data75[x,i] == t/10.:
                                    ...
                                    freq[t,2] = freq[t,2] + 1
                                    xi75ncar = np.append(xi75ncar, ncar_data75[x,i])
                                

for w in range(len(inhouse_data90[:,0])):  # NCAR 90
    for x in range(len(ncar_data90[:,0])):
        ################### PACIFIC ###################            
        if ((regions[0,0] <= ncar_data90[x,1] <= regions[0,1] and regions[0,2] <= ncar_data90[x,2] <= regions[0,3]) or ###Sierra Nevada
            (regions[1,0] <= ncar_data90[x,1] <= regions[1,1] and regions[1,2] <= ncar_data90[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
            (ncar_data90[x,1] >= regions[1,4] or ncar_data90[x,2] <= regions[1,5])):
                        ...
                        if inhouse_data[w,i] < 1000:
                            for t in range(11):
                                if ncar_data90[x,i] == t/10.:
                                    ...
                                    freq[t,5] = freq[t,5] + 1
                                    xi90ncar = np.append(xi90ncar, ncar_data90[x,i])
                                

for w in range(len(inhouse_data75[:,0])):  # sref 75
    for x in range(len(sref_data75[:,0])):
        ################### PACIFIC ###################            
        if ((regions[0,0] <= sref_data75[x,1] <= regions[0,1] and regions[0,2] <= sref_data75[x,2] <= regions[0,3]) or ###Sierra Nevada
            (regions[1,0] <= sref_data75[x,1] <= regions[1,1] and regions[1,2] <= sref_data75[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
            (sref_data75[x,1] >= regions[1,4] or sref_data75[x,2] <= regions[1,5])):
...
for w in range(len(inhouse_data90[:,0])):  # sref 90
    for x in range(len(sref_data90[:,0])):
                ################### PACIFIC ###################            
        if ((regions[0,0] <= sref_data90[x,1] <= regions[0,1] and regions[0,2] <= sref_data90[x,2] <= regions[0,3]) or ###Sierra Nevada
            (regions[1,0] <= sref_data90[x,1] <= regions[1,1] and regions[1,2] <= sref_data90[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
            (sref_data90[x,1] >= regions[1,4] or sref_data90[x,2] <= regions[1,5])):
...
###### Save Variables #######
#Frequency data
np.save('ncar_percentile_prob_pac_non_bias_corrected', freq)
np.save('sref_percentile_prob_pac_non_bias_corrected', freq_sref_bin)
#Skill Score data
np.save('ncar_percentile_prob_bss_pac_non_bias_corrected', bssf)
np.save('sref_percentile_prob_bss_pac_non_bias_corrected', bssf_sref)
##Uncertainty data
#np.save('ncar_sref_percentile_prob_uncertainty_con75_pac', pr_con75)
#np.save('ncar_sref_percentile_prob_uncertainty_unc75_pac', pr_unc75)
#np.save('ncar_sref_percentile_prob_uncertainty_con90_pac', pr_con90)
#np.save('ncar_sref_percentile_prob_uncertainty_unc90_pac', pr_unc90)
...
#
#ax1.errorbar(freq[:,0],freq[:,3], yerr= [abs(pr_unc75[0,:,1]-freq[:,3]), abs(pr_unc75[0,:,2]-freq[:,3])], c = 'b')
#ax1.errorbar(freq_sref_bin[:,0],freq_sref_bin[:,3], yerr= [abs(pr_unc75[1,:,1]-freq_sref_bin[:,3]), abs(pr_unc75[1,:,2]-freq_sref_bin[:,3])], c = 'r')
#
#ax1.errorbar(freq[:,0],freq[:,0], yerr= [abs(pr_con75[0,:,1]-freq[:,0]), abs(pr_con75[0,:,2]-freq[:,0])], c = 'b')
...
#
#ax2.errorbar(freq[:,0],freq[:,6], yerr= [abs(pr_unc90[0,:,1]-freq[:,6]), abs(pr_unc90[0,:,2]-freq[:,6])], c = 'b')
#ax2.errorbar(freq_sref_bin[:,4],freq_sref_bin[:,7], yerr= [abs(pr_unc90[1,:,1]-freq_sref_bin[:,7]), abs(pr_unc90[1,:,2]-freq_sref_bin[:,7])], c = 'r')
#
#ax2.errorbar(freq[:,0],freq[:,0], yerr= [abs(pr_con90[0,:,1]-freq[:,0]), abs(pr_con90[0,:,2]-freq[:,0])], c = 'b')
...
#plt.bar(freq[:,0],freq[:,2],width = .06, color = 'blue', edgecolor ='none', align='center')
#plt.xlim([-0.04,1.04])
#plt.ylim([10,10000])
...
#plt.bar(freq[:,0],freq[:,5],width = .06, color = 'blue', edgecolor ='none',align='center')
#plt.xlim([-0.04,1.04])
#plt.ylim([10,10000])
...
#
#
#plt.savefig("../../../public_html/reliability_diagram_sref_ncar_interp_bin_percentile_85_95_pac.pdf")                               
#        
#     
#          
#          
